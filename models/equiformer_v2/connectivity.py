...
class RadiusGraph(torch.nn.Module):
    ...
    def forward(
        ...
        edge_src = edge[1]
        degree = scatter_add(
            src=torch.ones_like(edge_dst), index=edge_dst, dim=0, dim_size=N_nodes
        )
...
class RadiusConnect(torch.nn.Module):
    def __init__(
        self, r: float, max_num_neighbors: int, offset: Optional[float] = None
    ):
        super().__init__()
        self.r: float = r
        self.max_num_neighbors: int = max_num_neighbors
...
class FpsPool(torch.nn.Module):
    def __init__(
        self, ratio: float, random_start: bool, r: float, max_num_neighbors: int
    ):
        super().__init__()
        ...
        self.r: float = r
        self.max_num_neighbors: int = max_num_neighbors
        self.radius_connect = RadiusConnect(
            r=self.r, max_num_neighbors=self.max_num_neighbors
        )

    def forward(
        self, node_coord_src: torch.Tensor, batch_src: torch.Tensor
    ) -> Tuple[
        ...
        if self.ratio != 1.0:
            ...
            node_dst_idx = torch.arange(
                0, len(node_coord_src), device=node_coord_src.device
            )
        ...
        edge_src, edge_dst = self.radius_connect(
