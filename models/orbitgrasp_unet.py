...
from equiformer_v2.gaussian_rbf import GaussianRadialBasisLayer, GaussianRadialBasisLayerFiniteCutoff
...
class EquiformerUnet(nn.Module):
    ...
    def __init__(
        ...
        self.sphere_linear_so3 = SO3_LinearV2(in_features=1, out_features=self.sphere_channels[0],
        ...
        for n in range(self.n_scales):
            ...
            block['pool'] = FpsPool(ratio=self.pool_ratio[n], random_start=not self.deterministic, r=self.max_radius[n],
                                    max_num_neighbors=self.max_neighbors[n])
            block['radius_graph'] = RadiusGraph(r=self.max_radius[n], max_num_neighbors=1000)
            ...
            if self.num_layers[n] > 1:
                if self.distance_function == 'gaussian':
                    down_distance_expansion = GaussianRadialBasisLayer(num_basis=self.num_distance_basis[n],
                                                                       cutoff=self.max_radius[n])
                elif self.distance_function == 'gaussian_soft':
        ...
        if self.distance_function == 'gaussian':
            middle_distance_expansion = GaussianRadialBasisLayer(num_basis=self.num_distance_basis[-1],
                                                                 cutoff=self.max_radius[-1])
        elif self.distance_function == 'gaussian_soft':
            middle_distance_expansion = GaussianRadialBasisLayerFiniteCutoff(
                num_basis=self.num_distance_basis[-1],
                cutoff=self.max_radius[-1] * 0.99)
        else:
            raise ValueError
        ...
        for i in range(self.num_layers[-1]):
            ...
            # Initialize the sizes of radial functions (input channels and 2 hidden channels)
            edge_channels_list = [int(self.num_distance_basis[-1])] + [self.edge_channels[-1]] * 2
        ...
        for n in range(self.n_scales - 1, -1, -1):
            ...
            if self.num_layers[n] > 1:
                if self.distance_function == 'gaussian':
                    up_distance_expansion = GaussianRadialBasisLayer(num_basis=self.num_distance_basis[n],
                                                                     cutoff=self.max_radius[n])
                elif self.distance_function == 'gaussian_soft':
        ...
        self.norm_2 = get_normalization_layer(self.norm_type, lmax=max(self.lmax_list),
                                              num_channels=self.sphere_channels[0])

        edge_channels_list = [int(self.num_distance_basis[0])] + [self.edge_channels[0]] * 2
    ...
    def forward(self, pcds):
        ...
        for n, block in enumerate(self.down_blocks):
            ...
            edge_distance_vec = node_coord.index_select(0, edge_src) - node_coord_dst.index_select(0, edge_dst)
            ...
            if len(block['layer_stack']) > 0:
                radiusGraph = block['radius_graph'](node_coord_src=node_coord,
                                                    node_feature_src=node_dst,
                                                    batch_src=batch)
                node_dst, node_coord_dst, edge_src, edge_dst, degree, batch_dst = radiusGraph
                ...
                for n, layer in enumerate(block['layer_stack'][1:]):
                    node_dst = layer['transblock'](node_src,
                                                   node_dst,
                                                   edge_attr,
                                                   edge_src,
                                                   edge_dst,
        ...
        node_src.embedding = (node_src.embedding + node_dst.embedding) / math.sqrt(3)  # Skip connection.
        ...
        for n, block in enumerate(self.up_blocks):
            if len(block['layer_stack']) != 0:
                ...
                for i, layer in enumerate(block['layer_stack'][1:]):
                    ...
                    edge_src, edge_dst = edge_dst, edge_src  # Swap source and destination.
                    node_dst.embedding = (node_src.embedding + node_dst.embedding) / math.sqrt(3)  # Skip connection.

                    edge_distance_vec = node_coord_dst.index_select(0, edge_src) - node_coord_dst.index_select(0,
                                                                                                               edge_dst)
                    edge_distance_vec = edge_distance_vec.detach()
                    edge_distance = torch.norm(edge_distance_vec, dim=-1).detach()
                    ...
                    node_dst = block['layer_stack'][i + 1]['transblock'](node_src,
                                                                         node_dst,
                                                                         edge_attr,
                                                                         edge_src,
                                                                         edge_dst,
            ...
            if n != self.n_scales - 1:
                ...
                edge_distance_vec = node_coord_dst.index_select(0, edge_src) - node_coord_src.index_select(0,
                                                                                                           edge_dst)
                edge_distance_vec = edge_distance_vec.detach()
                edge_distance = torch.norm(edge_distance_vec, dim=-1).detach()
        ...
        point_features = self.output_layer(node_src,
                                           node_dst,
                                           edge_attr,
                                           edge_src,
                                           edge_dst)
