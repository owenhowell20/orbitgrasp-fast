...
from data_generation_utils import get_gripper_angle_mask, show_pcds_masks, show_pcds_gt_segs, \
    voxel_down_sample_with_multiple_masks


def data_save(filename, mode, pcd_points, pcd_normals, all_masks, grasping_pose, success, object_num):
    assert mode in ['ab', 'wb']

    local_vars = locals()
    data_to_save = {k: local_vars[k] for k in
                    ['pcd_points', 'pcd_normals', 'all_masks', 'grasping_pose', 'success', 'object_num']}

    with open(filename, mode) as f:
        pickle.dump(data_to_save, f)  # type: ignore


def run(scene, object_set, iteration_num, savePath, from_save=True, start=0, GUI=True, n_intervals=36):
    sim = ClutterRemovalSim(scene, object_set, gui=GUI, rand=True, seed=12345, n_intervals=n_intervals)
    masks_ori = None
    ...
    for itr in trange(iteration_num, desc='Iteration'):
        if from_save:
            with open(Path(
                    __file__).resolve().parent / f'store/RawData_{scene}_multi/Indexes/index_{(itr + start):05}.pkl',
                      'rb') as f:
                while True:
                    try:
            ...
            with open(Path(
                    __file__).resolve().parent / f'store/RawData_{scene}_multi/Poses/pose_{(itr + start):05}.pkl', 'rb') as f:
                while True:
                    try:
            ...
            with open(Path(
                    __file__).resolve().parent / f'store/RawData_{scene}_multi/Masks/mask_{(itr + start):05}.pkl', 'rb') as f:
                while True:
                    try:
            ...
            with open(Path(
                    __file__).resolve().parent / f'store/RawData_{scene}_multi/Colors/color_{(itr + start):05}.pkl',
                      'rb') as f:
                while True:
                    try:
                        color = pickle.load(f)
        ...
        sim.reset(objs_num, index=object_index, pose_=position, color_=color, from_save=from_save)

        img = np.load(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Images/rgb_{itr + start:05}.npy')
        img_depth = np.load(
            Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Images/depth_{itr + start:05}.npy')
        ...
        for j, (masks_origin, colormap) in enumerate(zip(masks_ori, sim.colormaps)):
            start_idx = j * length
            end_idx = (j + 1) * length
            masks = filter_masks(masks_origin, sim.pcds_xyz[start_idx:end_idx], 0.1, bound_ratio=sim.bound_radio)
        ...
        for i, masks in enumerate(masks_filtered):
            ...
            if len(masks) != 0:
                ...
                pcds_masks.append(np.full((sim.colormaps[0].shape[0], sim.colormaps[0].shape[1]), False, dtype=bool))

            pcds_masks = np.asarray(pcds_masks).reshape(len(pcds_masks), -1, 1)[:,
                         sim.non_zero_indices[i * length:(i + 1) * length]]

            noise_indices = sim.pcds_xyz[i * length:(i + 1) * length, 2][
                                sim.non_zero_indices[i * length:(i + 1) * length]] < 0.052
            pcds_masks[:, noise_indices] = False
            pcds_gt_segs = seg_gts[i].reshape(-1, 1)[sim.non_zero_indices[i * length:(i + 1) * length]]
        ...
        all_points = np.vstack((np.asarray(pcds_all[0].points),
                                np.asarray(pcds_all[1].points),
                                np.asarray(pcds_all[2].points)))

        all_pcds_gt_segs = np.concatenate([all_pcds_gt_segs[i] for i in range(len(all_pcds_gt_segs))])
        combined_pcd = o3d.geometry.PointCloud()
        combined_pcd.points = o3d.utility.Vector3dVector(all_points)
        all_normals = np.vstack((np.asarray(pcds_all[0].normals),
        ...
        arr1 = np.pad(all_pcds_masks[0], ((0, 0), (0, b + c)), 'constant', constant_values=False)

        arr2 = np.pad(all_pcds_masks[1], ((0, 0), (a, c)), 'constant', constant_values=False)

        arr3 = np.pad(all_pcds_masks[2], ((0, 0), (a + b, 0)), 'constant', constant_values=False)

        all_masks = np.vstack((arr1, arr2, arr3))

        # Downsample the point cloud
        down_pcd, down_mask_list, down_gt_segs = voxel_down_sample_with_multiple_masks(combined_pcd, voxel_size=0.0055,
                                                                                       masks_arr=all_masks,
                                                                                       gt_seg_arr=all_pcds_gt_segs)

        points_array = np.asarray(down_pcd.points)
        filtered_down_mask_list = [downMask for downMask in down_mask_list if downMask.sum() > 0 and
                                   points_array[downMask][:, -1].mean() > 0.053]
        ...
        for order, down_mask in enumerate(down_mask_list):
            ...
            action_info = {'actions': actions_list, 'points': points_list, 'normals': normals_list,
                           'indices': indices_list, 'grasp_indices': select_idx[indices_list], 'order': order}
...
if __name__ == "__main__":
    ...
    run(scene, object_set, ITERATIONNUM, PATH, from_save=from_save, start=START, GUI=GUI, n_intervals=n_intervals)
