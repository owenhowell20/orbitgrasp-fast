...
def run(scene, object_set, iteration_num, savePath, from_save=True, start=0, GUI=True, n_intervals=36):
    ...
    for itr in trange(iteration_num, desc='Iteration'):
        if from_save:
            with open(
                    Path(
                        __file__).resolve().parent / f'store/RawData_{scene}_single/Indexes/index_{(itr + start):05}.pkl',
                    'rb') as f:
                while True:
                    try:
            ...
            with open(
                    Path(__file__).resolve().parent / f'store/RawData_{scene}_single/Poses/pose_{(itr + start):05}.pkl',
                    'rb') as f:
                while True:
                    try:
            ...
            with open(
                    Path(__file__).resolve().parent / f'store/RawData_{scene}_single/Masks/mask_{(itr + start):05}.pkl',
                    'rb') as f:
                while True:
                    try:
            ...
            with open(
                    Path(
                        __file__).resolve().parent / f'store/RawData_{scene}_single/Colors/color_{(itr + start):05}.pkl',
                    'rb') as f:
                while True:
                    try:
                        color = pickle.load(f)
            ...
            with open(
                    Path(
                        __file__).resolve().parent / f'store/RawData_{scene}_single/Camera/camera_{(itr + start):05}.pkl',
                    'rb') as f:
                while True:
                    try:
        ...
        img = np.load(Path(__file__).resolve().parent / f'store/RawData_{scene}_single/Images/rgb_{itr + start:05}.npy')
        img_depth = np.load(
            Path(__file__).resolve().parent / f'store/RawData_{scene}_single/Images/depth_{itr + start:05}.npy')
        ...
        pcds_all = list()
        index_list = list()
        masks = filter_masks(masks_ori, sim.pcds_xyz, 0.3, bound_ratio=sim.bound_radio, offset=15)
        ...
        pcds_masks = np.asarray(pcds_masks).reshape(len(pcds_masks), -1, 1)[:, sim.non_zero_indices]
...
if __name__ == "__main__":
    ...
    run(scene, object_set, ITERATIONNUM, PATH, from_save=args.from_save, start=START, GUI=GUI, n_intervals=n_intervals)
