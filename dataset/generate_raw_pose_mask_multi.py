...
def run(scene, start=0, iteration_num=200, lambda_p=4, gui=True, device='cuda:0'):
    env = ClutterRemovalSim(scene=scene, object_set='train', rand=True, gui=gui, seed=12345)
    total_num = 0
    total_mask = 0
    ...
    for i in range(iteration_num):
        ...
        if show_mask:
            ...
            for j, (masks_origin, colormap) in enumerate(zip(masks_list, colormaps)):
                ...
                ax.imshow(colormap)
                masks = filter_masks(masks_origin, env.pcds_xyz[j * length:(j + 1) * length], 0.1,
        ...
        # save
        if save_data:
            if not os.path.exists(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi'):
                os.makedirs(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi')
                os.makedirs(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Images')
                os.makedirs(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Masks')
                os.makedirs(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Indexes')
                os.makedirs(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Poses')
                os.makedirs(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Colors')

            np.save(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Images/rgb_{i + start:05}.npy',
                    colormaps)
            np.save(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Images/depth_{i + start:05}.npy',
                    depthmaps)
            with open(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Masks/mask_{i + start:05}.pkl',
                      'wb') as f:
                pickle.dump(masks_list, f)  # type: ignore

            with open(
                    Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Indexes/index_{i + start:05}.pkl',
                    'wb') as f:
                pickle.dump(indexes, f)  # type: ignore

            with open(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Poses/pose_{i + start:05}.pkl',
                      'wb') as f:
                pickle.dump(pose_list, f)  # type: ignore
            with open(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Colors/color_{i + start:05}.pkl',
                      'wb') as f:
...
def test(scene, i, from_save=True, gui=True):
    with open(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Indexes/index_{i:05}.pkl', 'rb') as f:
        while True:
            try:
                index = pickle.load(f)
    ...
    with open(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Poses/pose_{i:05}.pkl', 'rb') as f:
        while True:
            try:
                pose = pickle.load(f)
            except EOFError:  # End of File
    ...
    with open(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Masks/mask_{i:05}.pkl', 'rb') as f:
        while True:
            try:
    ...
    with open(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Colors/color_{i:05}.pkl', 'rb') as f:
        while True:
            try:
                color = pickle.load(f)
            except EOFError:  # End of File
    ...
    mask_num_ori = 0
    mask_num_filtered = 0
    for j, (colormap, depth, masks_origin) in enumerate(zip(colormaps, depthmaps, mask_list)):
        ...
        img = np.load(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Images/rgb_{i:05}.npy')
        img_depth = np.load(Path(__file__).resolve().parent / f'store/RawData_{scene}_multi/Images/depth_{i:05}.npy')
        ...
        if show_mask:
            ...
            masks_filtered = filter_masks(masks_origin, env.pcds_xyz[j * length:(j + 1) * length], 0.1,
                                          bound_ratio=env.bound_radio)
