...
def grasp_test_random(initial_radius=0.035, max_allowed_points=800, min_allowed_points=700,
    ...
    for RUN in range(config_file['test']['RUN_TIMES']):
        ...
        for _ in tqdm.tqdm(range(num_rounds), disable=silence):
            ...
            while sim.num_objects > 0 and consecutive_failures < 2 and skip_time < 3:
                ...
                pcd, ind_1 = pcd.remove_statistical_outlier(nb_neighbors=30, std_ratio=4.0)
                ...
                if len(data_list) == 0:
                    ...
                    print("dataset is 0")
                    continue
                score_list, feature_list = orbit_grasper.predict(data_list, sphere_indices_list,
