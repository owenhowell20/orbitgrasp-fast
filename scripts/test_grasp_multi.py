    ...
    for RUN in range(config_file["test"]["RUN_TIMES"]):
        ...
        for _ in tqdm.tqdm(range(num_rounds), disable=silence):
            ...
            while sim.num_objects > 0 and consecutive_failures < 2 and skip_time < 3:
                ...
                all_points = np.vstack([np.asarray(pcd.points) for pcd in pcds_all])
                all_normals = np.vstack([np.asarray(pcd.normals) for pcd in pcds_all])
                ...
                # this is a simple method to avoid memory overflow
                ...
                #
                #     grasp_normal = grasp_normal_list[l]
                #     for q in range(len(grasp_poses_list[l])):
                #
                #         grasp_pose = grasp_poses_list[l][q]
                #         pos = grasp_pose[:3]
                #         rot = grasp_pose[3:]
                #         gripper_angle_mask = get_gripper_angle_mask(Rotation.from_quat(rot), threshold=75)
                #
                #         if gripper_angle_mask:
                #             action = [pos, rot, grasp_normal]
                #             graspable, position_after_translation, rot = sim._decodeAction(action, z_thresh=0.052)
                #             if graspable == 0:
                #                 score_list[l][q] = -np.inf
                #                 continue
                #             else:
                ...
                #                 z_mask = get_gripper_points_mask(trans_matrix, threshold=0.052)
                #                 if not z_mask:
                #                     score_list[l][q] = -np.inf
                #                     continue
                #                 else:
                #                     filtered_grasp_poses[l][q] = np.concatenate([position_after_translation, rot])
                #         else:
                #             score_list[l][q] = -np.inf
        ...
        # else:
        #     np.save(Path.cwd() / 'test_gsr_dr_mask_random/seen_record_{}_{}_{}'.format(RUN, objs_num, policy_name), np.asarray(record))
