...
def show_anns(masks, ax, draw_bbox=True):
    ...
    for mask in masks:
        ...
        if draw_bbox:
            ...
            bbox = patches.Rectangle((x, y), w, h, linewidth=1, edgecolor=rgb_color, facecolor='None')
            ax.add_patch(bbox)
    ax.imshow(img)
...
def filter_masks(masks_list, pcds_xyz, iou_threshold=0.2, z_threshold=0.053, bound_ratio=1.5, offset=0, width_norm=48,
                 height_norm=40):
    if len(masks_list) == 0:
        return []
    ...
    for mask in masks_list:
        ...
        mask_points = pcds_xyz[non_zero_indices] * (mask['segmentation'].reshape(-1, 1)[non_zero_indices])
        ...
        if delta_height < height_bound[1] and delta_width < width_bound[1] and (
                delta_height > height_bound[0] and delta_width > width_bound[0]):
            keep = True
...
def get_mask_rgb(masks):
    mask_rgb = np.full((masks[0]['segmentation'].shape[0], masks[0]['segmentation'].shape[1], 3),
                       200,
                       dtype=np.uint8)
